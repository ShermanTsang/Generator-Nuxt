...
export default function({ store, $config }) {
  ...
  function getOssUrl(url, needProxy = false) {
    ...
    return isWithDomain
      ? is3rdResource
        ? url
        : `${resourceDomain}/${url.replace(/^http(s)?:\/\/(.*?)\//, "")}`
      : `${resourceDomain}/${url}`;
  }
  ...
  function getImageAsset(key, returnType = "value") {
    ...
    return returnType === "object"
      ? imageAssetTarget
      : this.$getOssUrl(imageAssetTarget.url) || "";
  }
  ...
  function getFileAsset(key, returnType = "value") {
    ...
    return returnType === "object"
      ? fileAssetTarget
      : this.$getOssUrl(fileAssetTarget.url) || "";
  }
  ...
  function fillStateByLocalStorage(
    actionKey,
    cacheKey,
    valueType,
    defaultValue
    ...
    if (process.client) {
      const localCache = localStorage.getItem(`<%= name %>~${cacheKey}`);
      let finalValue = null;
      if (!localCache) {
